# ---------------------------------------------
# Name:        dsq_functions.R
# Author:      Jess Maciuch
# Date:        2022-06-22
# Description: Useful functions for working with DSQ-1 data
# ---------------------------------------------
...
#--------------------------------------------------------------------------#
## Function: response_rate

# Purpose: Determine percentage of missing values. 

# Input: DSQ data in dataframe form (i.e. from pull_DSQ() )

# Output: A dataframe containing rate of missing values as a decimal. 

response_rate <- function(dataset) {
  output <- data.frame()
  ...
  for (j in 1:ncol(dataset)) {
    ...
    miss.rate <- k / nrow(dataset)
    res.rate <- (1 - miss.rate) * 100
    output[1, j] <- res.rate
    colnames(output)[j] <- colnames(dataset)[j]
  }
  return(output)
}

#--------------------------------------------------------------------------#
## Function: symptom_present

# Purpose: Determine if symptom meets 2x2 threshold for frequency and severity.

# Input: DSQ data in dataframe form (i.e. from pull_DSQ() )

# Output: A dataframe containing all symptoms present in dsq.data.
# '1' if symptom present, '0' if not, 'NA' if either value missing. 

symptom_present <- function(dsq.data) {
  symptoms <- data.frame()
  
  for (j in seq(from = 1, to = ncol(dsq.data), by = 2)) {
    name <- colnames(dsq.data)[j]
    name %<>% substr(., 1, nchar(.)-1)
    
    for (i in 1:nrow(dsq.data)) {
      f <- dsq.data[[i, j]]
      s <- dsq.data[[i, j+1]]
...
# i.e. replace any value that isn't a 0-4 with NA

# Input: "dsq_data" must be a data frame or matrix

clean_dsq <- function(dataset) {
  norm_vec <- list("0", "1", "2", "3", "4")
  
  for (j in 1:ncol(dataset)) {
    for (i in 1:nrow(dataset)) {
      
      # Convert values that are not 0-4 into NA (i.e. missing values in R)
      if (!dataset[i, j] %in% norm_vec) {
        dataset[i, j] <- NA
      } 
    }
  }
  ...
  dataset %<>% 
    mutate(across(1:ncol(dataset), as.numeric))
...
## Function: pull_dsq_shortform

# Purpose: pull responses to DSQ short form questions

# Input: "dataset" must be a data frame or matrix
# e.g. produced by read_csv()

pull_dsq_shortform <- function(dataset) {
  names <- c("fatigue13f",
             "fatigue13s",
             "soreness15f",
             "soreness15s",
...
# Purpose: pull responses to DSQ-1 (AND ONLY DSQ-1) questions from dataset

# Input: "dataset" must be a data frame or matrix
# e.g. produced by read_csv()

pull_dsq <- function(dataset) {
  names <- c("fatigue13f",
             "fatigue13s",
             "heavy14f",
             "heavy14s",
  ...
  # Select only columns that match names vector
  output <- names %>%
    all_of(.) %>%
    select(dataset, .) %>%
    clean_dsq
...
# Purpose: pull responses to DSQ 2 questions from dataset

# Input: "dataset" must be a data frame or matrix
# e.g. produced by read_csv()

pull_dsq_2 <- function(dataset) {
  names <- c("fatigue13f",
             "fatigue13s",
             "heavy14f",
             "heavy14s",
...
# Purpose: pull neurocognitive domain DSQ-1 items from dataset

# Input: "dataset" must be a data frame or matrix
# e.g. produced by read_csv()

# Notes: Domain items pulled from Jason et al. 2021 
# doi:10.1080/21641846.2021.1922140.

pull_neuro <- function(dataset) {
  names <- c("remember36f",
             "remember36s",
             "difficulty37f",
             "difficulty37s",
...
# Purpose: pull PEM domain DSQ-1 items from dataset
...
# Purpose: calculate composite score from frequency and severity DSQ questions

# Input: "dsq_data" must be output from pull_dsq 

composite_score <- function(dsq_data) {
  k <- 1               # Column counter for new comp score data frame
  comp_data <- data.frame()
  
  for (j in seq(from = 1, to = ncol(dsq_data), by = 2)) {
    for (i in 1:nrow(dsq_data)) {
      f <- dsq_data[[i, j]]
      s <- dsq_data[[i, j+1]]
      
      comp_data[i, k] <- mean(c(f, s)) * 25
    }
    
    symptom_name <- colnames(dsq_data)[j]
    symptom_name %<>% substr(., 1, nchar(.)-1)
    
    colnames(comp_data)[k] <- paste(symptom_name, "c", sep = "")
...
## Function: diagnose_MECFS

# Purpose: Determine if respondent meets Fukuda, CCC, ME-ICC, and IOM case 
# definitions. 

# Input: "dataset" must be data frame (i.e. from read_csv)

# Notes: Diagnostic guidelines taken from Jason & Sunnquist (2018) Supplement A:
# https://www.leonardjason.com/wp-content/uploads/2018/11/Jason-Sunnquist-2018.-The-Development...Suppl-A-DSQ-1.pdf

diagnose_MECFS <- function(dataset) {
  ...
  for (m in 1:ncol(add_data)) {
    for (n in 1:nrow(add_data)) {
      if (add_data[[n, m]] == "#NULL!") {
        add_data[n, m] <- NA
      } 
    }
  }
  ...
  add_data %<>% 
    mutate(across(1:ncol(add_data), as.numeric))
  
  # Pull all data needed for case definition assessment
  diagnostics <- cbind(dataset$"ID",
                       SF36$RP,
                       SF36$SF,
                       SF36$VT,
                       dsq,
  ...
  colnames(diagnostics)[1] <- "ID"
  colnames(diagnostics)[2] <- "RP"
  colnames(diagnostics)[3] <- "SF"
  colnames(diagnostics)[4] <- "VT"
  
  # Test if ppt meets case definitions 
  case_definitions <- data.frame(ID = NA,
                                 fukuda = NA,
                                 CCC = NA,
                                 ME_ICC = NA,
                                 IOM = NA)
  ...
  for (i in 1:nrow(diagnostics)) {
    response <- diagnostics[i,]
    
    ID <- diagnostics[i, 'ID']
    fukuda <- diagnose_fukuda(response)
    CCC <- diagnose_CCC(response)
    ME_ICC <- diagnose_ME_ICC(response)
    IOM <- diagnose_IOM(response)
    
    case_definitions[i,] <- c(ID, 
                              fukuda,
                              CCC,
                              ME_ICC,
                              IOM)
